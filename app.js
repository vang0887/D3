...
function updateToolTip(chosenXAxis, chosenYAxis, circlesGroup) {
  ...
  var toolTip = d3.tip()
    ...
    .html(function(d) {
        ...
        var yLine = `<div>${chosenXAxis}: ${d[chosenXAxis]}%</div>`;
        if (chosenXAxis === "poverty") {
            xLine = `<div>${chosenXAxis}: ${d[chosenXAxis]}%</div>`}          
        else {
            xLine = `<div>${chosenXAxis}: ${parseFloat(d[chosenXAxis]).toLocaleString("en")}</div>`;}             
...
d3.csv("Data.csv").then(function(CSVdata, err) {
  if (err) throw err;
